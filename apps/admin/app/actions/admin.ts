...
export async function toggleQuizPublic(id: string) {
  try {
    const admin = await ensureAdmin();
    const quiz = await db.quiz.findUnique({ where: { id }, select: { isPublic: true, title: true, testSeriesId: true } });
    if (!quiz) return { error: "Quiz not found." };

    // Making a series test public would put it on the anonymous /test/[slug] route,
    // where none of the series access rules apply — and once series are sold, that
    // is simply giving away paid content. Only the unsafe direction is blocked:
    // clearing the flag on a quiz that is already public must always be possible,
    // since some series quizzes were flagged public before this rule existed.
    if (quiz.testSeriesId && !quiz.isPublic) {
      return { error: "Tests that belong to a series cannot be made public." };
    }
